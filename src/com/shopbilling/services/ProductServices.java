...
public class ProductServices {
	...
	private static final String INS_PRODUCT = "INSERT INTO PRODUCT_DETAILS (PRODUCT_ID,PRODUCT_NAME,MEASURE,QUANTITY,PURCHASE_PRICE,"
			+ "SELL_PRICE,PRODUCT_MRP,DISCOUNT,ENTRY_DATE,LAST_UPDATE_DATE,DESCRIPTION,ENTER_BY,CATEGORY_ID,PURCHASE_RATE,PRODUCT_TAX,BAR_CODE,SUPPLIER_ID)"
			+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	private static final String SELECT_PRODUCT = "SELECT  PD.PRODUCT_ID,PD.PRODUCT_NAME,PD.MEASURE,PD.QUANTITY,PD.PURCHASE_PRICE,PD.SELL_PRICE,"
			+ "PD.PRODUCT_MRP,PD.DISCOUNT,PD.ENTRY_DATE,PD.LAST_UPDATE_DATE,PD.DESCRIPTION,PD.ENTER_BY,"
			+ "PD.PURCHASE_RATE,PD.PRODUCT_TAX,PD.BAR_CODE,PCD.CATEGORY_NAME,PD.SUPPLIER_ID,SD.SUPPLIER_NAME "
			+ "FROM PRODUCT_DETAILS PD,PRODUCT_CATEGORY_DETAILS PCD,SUPPLIER_DETAILS SD WHERE PD.CATEGORY_ID = PCD.CATEGORY_ID AND PD.SUPPLIER_ID=SD.SUPPLIER_ID AND PRODUCT_ID=?;";
	...
	private static final String UPDATE_PRODUCT = "UPDATE PRODUCT_DETAILS SET PRODUCT_NAME=?,MEASURE=?,QUANTITY=?,PURCHASE_PRICE=?,"
			...
			+ "ENTER_BY=?,CATEGORY_ID=?,PURCHASE_RATE=?,PRODUCT_TAX=?,BAR_CODE=?,SUPPLIER_ID=? WHERE PRODUCT_ID=?";
	...
	public static List<Product> getAllProducts() {
		...
		try {
			...
			while (rs.next()) {
				...
				pc.setQuanity(rs.getDouble("QUANTITY"));
	...
	public static List<Product> getAllProductsBySupplier(Integer supplierId) {
		Connection conn = null;
		PreparedStatement stmt = null;
		Product pc = null;
		List<Product> productList = new ArrayList<Product>();
		try {
			StringBuilder GET_ALL_PRODUCTS_BY_SUPPLIER = new StringBuilder(
					"SELECT  PD.PRODUCT_ID,PD.PRODUCT_NAME,PD.MEASURE,PD.QUANTITY,PD.PURCHASE_PRICE,PD.SELL_PRICE,PD.PRODUCT_MRP,PD.DISCOUNT,PD.ENTRY_DATE,PD.LAST_UPDATE_DATE,PD.DESCRIPTION,PD.ENTER_BY,PD.PURCHASE_RATE,PD.PRODUCT_TAX,PD.BAR_CODE,PCD.CATEGORY_NAME,SD.SUPPLIER_NAME,SD.SUPPLIER_ID FROM PRODUCT_DETAILS PD,PRODUCT_CATEGORY_DETAILS PCD,SUPPLIER_DETAILS SD WHERE PD.CATEGORY_ID = PCD.CATEGORY_ID AND PD.SUPPLIER_ID=SD.SUPPLIER_ID ");
			String supplierQuery = "AND PD.SUPPLIER_ID=?";
			conn = PDFUtils.getConnection();
			if(supplierId!=null) {
				GET_ALL_PRODUCTS_BY_SUPPLIER.append(supplierQuery);
			}
			stmt = conn.prepareStatement(GET_ALL_PRODUCTS_BY_SUPPLIER.toString());
	...
	public static StatusDTO addProduct(Product product) {
		...
		try {
			if (product != null) {
				...
				stmt.setDouble(4, product.getQuanity());
				...
				stmt.setInt(17, product.getSupplierId());
	...
	public static StatusDTO updateProduct(Product product) {
		...
		try {
			if (product != null) {
				...
				stmt.setLong(15, product.getSupplierId());
	...
	public static StatusDTO saveBillDetails(BillDetails bill) {
		...
		try {
			if (bill != null) {
				...
				stmt.setDouble(6, bill.getTotalQuanity());
	...
	public static List<BillDetails> getBillDetails(Date fromDate, Date toDate, Long customerMobile,
		...
		StringBuilder SELECT_BILL_DETAILS = new StringBuilder(
				"SELECT CBD.*,CD.CUST_NAME AS CUSTOMER_NAME,SD.NAME AS SALESMAN_NAME FROM CUSTOMER_BILL_DETAILS CBD,CUSTOMER_DETAILS CD,SALESMAN_DETAILS SD WHERE DATE(CBD.BILL_DATE_TIME) BETWEEN ? AND ?  "
		...
		try {
			...
			while (rs.next()) {
				...
				billDetails.setTotalQuanity(rs.getDouble("BILL_QUANTITY"));
	...
	public static boolean quickStockCorrection(Product product) {
		...
		try {
			if (product != null) {
				...
				stmt.setInt(2, product.getProductCode());
				stmt.setDouble(1, product.getQuanity());
	...
	public static BillDetails getBillDetailsOfBillNumberWithinDateRange(Integer billNumber, Date fromDate,
			Date toDate) {
		Connection conn = null;
		PreparedStatement stmt = null;
		BillDetails billDetails = null;
